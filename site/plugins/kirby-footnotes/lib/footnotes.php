...
class Footnotes {
    ...
    public static function convert($text, $remove = false, $without = false, $only = false, $kt = true, $startAt = 1, $unwrapped = false, $containerInt = 1) {
        ...
        if(preg_match_all('/\[(\^.*?)\]/s', $text, $matches)) {
            ...
            foreach($notes as $key => $note) {
                $data       = ['containerInt' => $containerInt, 'count' => $count, 'order' => $order, 'note' => $note];
